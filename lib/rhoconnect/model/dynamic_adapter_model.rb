...
module Rhoconnect
  module Model
    class DynamicAdapterModel < Rhoconnect::Model::Base
      ...
      def self.store_blob(object,field_name,blob)
        xml = '<?xml version="1.0" encoding="ISO-8859-1"?> <file version="1.0">'
        xml << "<attributes DateCreated=\"#{Time.now}\" size=\"#{blob[:tempfile].size}\"/>"
        xml << "<name encoding=\"none\">#{File.basename(object[field_name])}</name>"
        xml << "<location encoding=\"none\">#{File.dirname(object[field_name])}</location>"
        xml << '<content encoding="base64">'
        xml << Base64.encode64(blob[:tempfile].read)
        xml << '</content>'
        xml << '</file>'
      end
